...
static ggml_backend_buffer_type_t ggml_backend_ane_device_get_host_buffer_type(ggml_backend_dev_t dev) {
    // Return nullptr to avoid being treated as a host buffer by the model loader.
    // The model loader switches host buffers to CPU when use_mmap=true, which we don't want.
    // ANE can still use buffer_from_host_ptr via the device interface directly.
    return nullptr;
    GGML_UNUSED(dev);
}
...
static ggml_backend_buffer_t ggml_backend_ane_device_buffer_from_host_ptr(ggml_backend_dev_t dev, void * ptr, size_t size, size_t max_tensor_size) {
    ...
    fprintf(stderr, "[ANE DEVICE] buffer_from_host_ptr CALLED: ptr=%p, size=%.2f MB, max_tensor_size=%.2f MB\n", 
            ptr, size / (1024.0 * 1024.0), max_tensor_size / (1024.0 * 1024.0));
    fprintf(stderr, "[ANE DEVICE] This wraps existing mmap'd GGUF memory - NO COPY!\n");
